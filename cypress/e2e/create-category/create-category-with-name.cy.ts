describe('Create Category With Name', () => {
  const createNewPersonalCategory = () => {
    ...
    cy.get('div[id="category-item-new cat"]').should('not.exist')
    ...
    cy.get('div#personal-calendar-accordion-item').should('be.visible')
    cy.get('div#personal-calendar-accordion-item').eq(0).children().should('contain', 'new cat')
    cy.get('div[id="category-item-new cat"]').should('be.visible')
  }
